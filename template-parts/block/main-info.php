    ...
    <?php while( have_rows('main-info') ) : the_row(); $products = get_sub_field('products_repeater'); // Получаем товары из ACF?>
        ...
        <div class="main-info<?php if($count % 2 !== 0 ){echo' main-info--reverse';}?>">
            ...
            <div class="main-info__content">
                ...
                if ($products):
                    ...
                    if ($loop->have_posts()): ?>
                        <div class="flex gap-26 mt-30 sm-flex-col slider-default slider-main-info owl-carousel">
        ...
        <span class="product-card__preview__stickers__wish btn-modal-open--wish-add wish-btn item-card__btn-wish<?php if(getCookiesArr()){if (in_array(get_the_ID(), getCookiesArr())) { echo ' active'; }} ?>" data-id="<?php echo get_the_ID(); ?>" data-title="<?php the_title();?>">
            ...
            <?php } ?>
                                        ...
                                        <div class="product-card__btns sm-hidden">
                                            <a href="<?php echo esc_url( $link_url ); ?>"  target="<?php echo esc_attr( $link_target ); ?>" class="product-card__btns__main">Перейти в каталог</a>
                                        </div>
                                    </div>

                                    <div class="product-card__btns lg-hidden sm-mt-11">
                                        <a href="<?php echo esc_url( $link_url ); ?>"  target="<?php echo esc_attr( $link_target ); ?>" class="product-card__btns__main">Перейти в каталог</a>
                                    </div>
                                </div>
                            <?php endwhile; ?>
